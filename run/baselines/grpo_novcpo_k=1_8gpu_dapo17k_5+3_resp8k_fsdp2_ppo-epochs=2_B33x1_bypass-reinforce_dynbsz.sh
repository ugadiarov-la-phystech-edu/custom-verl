#!/usr/bin/env bash
# grpo_novcpo_k=1_8gpu_dapo17k_5+3_resp8k_fsdp2_ppo-epochs=2_B33x1_bypass-reinforce_dynbsz.sh
#
# FSDP2 counterpart of ..._megatron_offload_..._dynbsz.sh. Data, layout, loss, schedule and
# staleness are identical; ONLY the training backend differs, so the two are directly
# comparable. Megatron is also the more constrained path -- verl/models/mcore/registry.py
# dispatches on the HF architecture string and covers ~16 architectures, while FSDP2 loads
# any AutoModelForCausalLM.
#
...
# ================= Logging =================
exp_name=${exp_name:-"GRPO-noVCPO-v080 bypass-reinforce dynbsz k-${staleness_threshold} DAPO17K-AIME24 Qwen3-8B ${n_gpus_rollout}-${n_gpus_training} fsdp2 B-${train_prompt_mini_bsz}x${num_minibatches_per_update} ppo-epochs-${ppo_epochs} ${loss_agg_mode} ${max_response_length}-len ${weight_decay}-wd"}
