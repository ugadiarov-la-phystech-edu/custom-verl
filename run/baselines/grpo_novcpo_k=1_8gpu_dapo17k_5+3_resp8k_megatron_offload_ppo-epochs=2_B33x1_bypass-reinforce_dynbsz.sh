#!/usr/bin/env bash
# grpo_novcpo_k=1_8gpu_dapo17k_5+3_resp8k_megatron_offload_ppo-epochs=2_B33x1_bypass-reinforce_dynbsz.sh
#
# Sibling of the ..._bypass-reinforce.sh arms with TWO changes: staleness k=1 and
# DYNAMIC BATCH SIZE on the trainer. The loss, layout, optimizer offload and checkpoint
...
# ================= Logging =================
exp_name=${exp_name:-"GRPO-noVCPO-v080 bypass-reinforce dynbsz k-${staleness_threshold} DAPO17K-AIME24 Qwen3-8B ${n_gpus_rollout}-${n_gpus_training} tp1dp3 hdo B-${train_prompt_mini_bsz}x${num_minibatches_per_update} ppo-epochs-${ppo_epochs} ${loss_agg_mode} ${max_response_length}-len ${weight_decay}-wd"}
