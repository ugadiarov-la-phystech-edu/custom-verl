#!/usr/bin/env bash
# grpo_novcpo_k=0.5_8gpu_dapo17k_5+3_resp8k_megatron_offload_ppo-epochs=2_B33x1_verl-default-loss.sh
#
# Sibling of ..._k=2_..._verl-default-loss.sh with a TIGHTER STALENESS GATE (k=0.5).
# Together with ..._k=0.5_..._bypass-reinforce.sh and ..._k=0.5_..._decoupled-is.sh this
# completes a three-way LOSS comparison at fixed staleness: stock PPO-clip against
# pi_rollout (here) / IS-weighted REINFORCE / decoupled PPO with IS.
...
#     at most ~1.5 param-versions old instead of ~3. This matters more for THIS loss
#     than for the others: the clip anchor here is pi_rollout, so staleness enters the
#     PPO ratio directly and is clipped as if it were policy movement.
