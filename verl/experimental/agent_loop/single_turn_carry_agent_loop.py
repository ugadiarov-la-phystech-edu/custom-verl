...
class PartialCarryAgentLoop(SingleTurnAgentLoop):
    ...
    async def run(self, sampling_params: dict[str, Any], **kwargs) -> AgentLoopOutput:
        ...
        # 1. extract multimodal inputs from messages (same as the single-turn loop)
        ...
        # 2. apply chat template and tokenize the *original* prompt (deterministic; reproduces the
        #    prompt token ids from the first step so the prompt/response split stays canonical)
        ...
        # 3. carry-over state injected by the trainer (absent -> plain single-turn behavior)
        ...
        # 4. resume generation by re-prefilling prompt + tokens so far, capped to the remaining budget.
        #    An already-finished rollout (carry_done) or one that has hit response_length passes through.
        ...
        if not carry_done and remaining > 0:
            ...
            # Schedule carried partials ahead of fresh requests (vllm + scheduling_policy=priority
            # only; None means the backend takes no priority kwarg, so omit it).
        ...
        # stop_reason lets the trainer tell a finished rollout (eos/length/stop) from an aborted one
        # (continue next step). turn_scores/tool_rewards keep the schema aligned with the tool loop.
